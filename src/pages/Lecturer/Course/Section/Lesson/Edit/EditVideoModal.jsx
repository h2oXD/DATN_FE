...
export default function EditVideoModal({ showVideoModal, setShowVideoModal, lesson, section_id, setLessons }) {
    ...
    const formik = useFormik({
        initialValues: {
            ...
            video_url: lesson.videos.video_url,
        ...
        onSubmit: async (values) => {
            setLoading(true)
            const updateLesson = { title: values.title, description: values.description, type: 'video', _method: 'PUT' }
            try {
                const res = await axiosClient.put(`lecturer/courses/${course_id}/sections/${section_id}/lessons/${lesson.id}`, updateLesson)
                await axiosClient.put(`lecturer/courses/${course_id}/sections/${section_id}/lessons/${lesson.id}/videos/${lesson.videos[0].id}`, uploadVideo, {
                    headers: {
    ...
    return (
        <Modal width={1000} open={showVideoModal} onCancel={() => { setShowVideoModal(false); formik.resetForm(); formik.setFieldValue("description", '   '); setVideoPreviewUrl(null) }} footer={null}>
